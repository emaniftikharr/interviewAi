...
from history import delete_session, get_topic_stats, init_db, load_sessions, save_session
...
def _end_session() -> None:
    ...
    try:
        save_session(
            role=ss.role, topic=ss.topic, difficulty=ss.difficulty,
            mode=ss.mode, scores=ss.scores[:], qa_history=ss.qa_history,
        )
    except Exception:
...
def _render_history_page() -> None:
    ...
    with c_theme:
        icon = "☀️" if ss.theme == "dark" else "🌙"
        if st.button(icon, key="_hist_theme", help="Toggle theme"):
            ss.theme = "light" if ss.theme == "dark" else "dark"; st.rerun()

    st.markdown("""
<div class="aip-header">
  <div class="aip-logo">📜 Session History</div>
  <div class="aip-sub">All your past interview sessions</div>
</div>
""", unsafe_allow_html=True)

    sessions = load_sessions()
    if not sessions:
        st.info("No sessions saved yet. Complete an interview to see your history here.")
    ...
    c2.metric("Overall Avg", f"{sum(avg_scores)/len(avg_scores):.1f}/10")
    c3.metric("Personal Best", f"{max(avg_scores):.1f}/10")
    c4.metric("Topics Covered", len({s["topic"] for s in sessions}))

    st.divider()
    st.markdown("### 🗂️ Past Sessions")
    for s in sessions:
        scores_list = json.loads(s["scores"]) if isinstance(s["scores"], str) else s["scores"]
        sc_emoji = get_score_emoji(round(s["avg_score"]))
        diff_emoji = get_difficulty_emoji(s["difficulty"])
        with st.expander(
            f"{sc_emoji}  {s['topic']}  ·  {s['role']}  ·  "
            f"{diff_emoji} {s['difficulty']}  ·  **{s['avg_score']:.1f}/10**  ·  {s['created_at']}"
        ):
            mc1, mc2, mc3, mc4 = st.columns(4)
            mc1.metric("Avg Score", f"{s['avg_score']:.1f}/10")
            mc2.metric("Questions", s["total_qs"])
            mc3.metric("Mode", s["mode"])
            mc4.metric("Difficulty", s["difficulty"])
            if scores_list:
                st.markdown(
                    '<p style="font-family:Inter,sans-serif;font-size:.7rem;color:var(--t4);'
                    'text-transform:uppercase;letter-spacing:.07em;margin:.5rem 0 .15rem;">'
                    'Score per question</p>',
                    unsafe_allow_html=True,
                )
